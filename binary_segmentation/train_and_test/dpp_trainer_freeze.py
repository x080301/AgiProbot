...
class BinarySegmentationDPP:
    freeze_parameters = ['bn5.weight', 'bn5.bias', 'conv5.weight', 'conv5.bias',
                         'bn6.weight', 'bn6.bias', 'conv6.weight', 'conv6.bias',
    ...
    def train(self, rank, world_size):
        ...
        for name, param in model.named_parameters():
            if name in self.freeze_parameters:
        ...
        if self.args.use_sgd:
            opt = torch.optim.SGD([{'params': parameters_to_train, 'initial_lr': self.args.lr}],
                                  lr=self.args.lr,
                                  momentum=self.args.momentum, weight_decay=1e-4)
        else:
            opt = torch.optim.Adam(parameters_to_train, lr=self.args.lr, weight_decay=1e-4)
