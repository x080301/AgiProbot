...
def train(args, io):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (points, target, goals, masks) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
            points, target, goals, masks = points.to(device), target.to(device), goals.to(device), masks.to(
            ...
            batch_label = target.view(-1, 1)[:,
                          0].cpu().data.numpy()  # array(batch_size*num_points)            loss = criterion(seg_pred.view(-1, NUM_CLASS), target.view(-1,1).squeeze(),weights,using_weight=args.use_weigth)     #a scalar
            loss = criterion(seg_pred.view(-1, NUM_CLASS), target.view(-1, 1).squeeze(), weights__,
        ...
        with torch.no_grad():
            ...
            for i, (points, seg) in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):
                ...
                loss = criterion(seg_pred_.view(-1, NUM_CLASS), seg.view(-1, 1).squeeze(), weights__,
                                 using_weight=args.use_class_weight)  # a scalar
            ...
            outstr_without_background = 'Validation without backgroud----epoch: %d, mIoU %.6f,  eval point acc: %.6f, eval avg class acc: %.6f' % (
            epoch, noBG_mIoU,
            (sum(noBG_correct_class) / float(sum(noBG_seen_class))),
