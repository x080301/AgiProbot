...
def download():
    ...
    if not os.path.exists(os.path.join(DATA_DIR, 'h5_files')):
        ...
        print('wget %s  --no-check-certificate; unzip %s' % (www, zipfile))
...
def load_scanobjectnn_data(partition):
    ...
    h5_name = os.getcwd() + '/data/h5_files/main_split/' + partition + '_objectdataset_augmentedrot_scale75.h5'
...
class ScanObjectNN(Dataset):
    ...
    def __getitem__(self, item):
        ...
        label = F.one_hot(torch.tensor([label]), num_classes=15)
