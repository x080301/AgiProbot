...
def sort_chunk_nonuniform(attention_point_score, bin_boundaries):
    ...
    for i in range(num_bins):
        ...
        for j in range(B):
            if i == 0:
                index_in_bin = torch.where(attention_point_score[j, 0, :] > bin_boundaries[i])
            elif i < num_bins - 1:
                index_in_bin = torch.where(
                    (attention_point_score[j, 0, :] > bin_boundaries[i]) + (
                            attention_point_score[j, 0, :] < bin_boundaries[i - 1]))
            else:
                index_in_bin = torch.where(attention_point_score[j, 0, :] < bin_boundaries[i - 1])
