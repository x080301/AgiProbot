...
def vis_data_name_init(config, based_config=True):
    ...
    elif config.neighbor2point_block.downsample.ds_which == "global_carve":
        new = {
            "col_sum": [],
            "row_std": [],
            "sparse_row_sum": [],
    ...
    elif config.neighbor2point_block.downsample.ds_which == "local":
        new = {
            "local_std": []
        }
    elif config.neighbor2point_block.downsample.ds_which == "local_insert":
        new = {
            "local_std": [],
            "sparse_row_std": [],
            "sparse_col_sum": [],
...
def vis_data_structure_init(config, based_config=False):    
    ...
    elif config.neighbor2point_block.downsample.boltzmann.enable[0]:
        trained = {
            "idx": [],
            "attention_point_score":[],
            "aps_boltz": []
...
def vis_data_extract(config, model):
    ...
    if config.datasets.dataset_name == "shapenet_AnTao350M" or config.datasets.dataset_name == "shapenet_Yi650M" or config.datasets.dataset_name == "shapenet_Normal":
        for i in range(len(config.neighbor2point_block.downsample.M)):
            ...
            vis_dict["trained"]["attention_point_score"].append(model.module.block.downsample_list[i].attention_point_score)
            if config.neighbor2point_block.downsample.bin.enable[i]:
                bin_prob = model.module.block.downsample_list[i].bin_prob
                vis_dict["trained"]["bin_prob"].append(bin_prob.unsqueeze(1))
            elif config.neighbor2point_block.downsample.boltzmann.enable[i]:
        ...
        if config.neighbor2point_block.downsample.ds_which == "global_carve":
            for i in range(len(config.neighbor2point_block.downsample.M)):
                # trained model
                attention_map = model.module.block.downsample_list[i].attention_map
                mask = model.module.block.downsample_list[i].mask
                ...
                sparse_attention_map_std = sparse_attention_map.masked_select(mask!=0).view(sparse_attention_map.shape[:-1] + (model.module.block.downsample_list[i].K,))
                ...
                for mode, value in vis_dict["heatmap"].items():
                    if config.test.few_points.enable:
                        ...
                        num_points = config.neighbor2point_block.downsample.M[i]
            ...
            raise ValueError(f'Only support local_insert and global_carve downsample mode! Got: {config.datasets.dataset_name}')
    else:
...
def vis_data_gather(config, model, device, rank, vis_test_gather_dict):
    ...
    # else:
    #     idx_modes = [config.neighbor2point_block.downsample.idx_mode[0]]
    for vis_type in vis_test_gather_dict.keys():
        for idx_mode in vis_test_gather_dict[vis_type].keys():
            gather_list = [[torch.empty_like(vis_all_dict[vis_type][idx_mode][j]).to(device) for _ in range(config.test.ddp.nproc_this_node)] for j in range(len(config.neighbor2point_block.downsample.M))]
            for j in range(len(config.neighbor2point_block.downsample.M)):
    ...
    for vis_type in vis_test_gather_dict.keys():
        for idx_mode in vis_test_gather_dict[vis_type].keys():
            for j in range(len_ds):
                vis_concat_dict[vis_type][idx_mode].append(np.concatenate(vis_test_gather_dict[vis_type][idx_mode][j], axis=0))
