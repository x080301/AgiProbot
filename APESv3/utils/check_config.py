...
def check_config(config):
    ...
    idx_mode_dict = {
        "global_carve": ["col_sum", "row_std", "sparse_row_sum", "sparse_row_std", "sparse_col_sum", "sparse_col_avg", "sparse_col_sqr"],
        "local": ["local_std"],
        "global": ["col_sum"],
    ...
    # train & test
        # gpu
    assert config.train.ddp.nproc_this_node == config.train.ddp.world_size == len(config.train.ddp.which_gpu), "Train GPU settings must match each other!"
    assert config.test.ddp.nproc_this_node == config.test.ddp.world_size == len(config.test.ddp.which_gpu), "Test GPU settings must match each other!"
    ...
    if config.test.visualize_combine.enable:
        ...
        for idx_mode in config.test.visualize_combine.vis_which:
            if idx_mode not in idx_mode_dict[config.neighbor2point_block.downsample.ds_which]:
                raise ValueError(f"When visualize_combine is enabled, vis_which should be one of {idx_mode_dict[config.neighbor2point_block.downsample.ds_which]}! Got: {idx_mode}")
    ...
    if config.neighbor2point_block.embedding.normal_channel + (config.datasets.dataset_name == "shapenet_Normal") == 1:
        ...
        assert config.neighbor2point_block.embedding.conv1_in[0] == 12, "When use dataset with normal vector, the first conv_in must be 12"
    else:
        assert config.neighbor2point_block.embedding.conv1_in[0] == 6, "When didn't use dataset with normal vector, the first conv_in must be 6"
...
def get_gpu_info():
    try:
        result = subprocess.run(['nvidia-smi', '--query-gpu=index,power.draw,memory.used,utilization.gpu,memory.free', '--format=csv,noheader,nounits'], stdout=subprocess.PIPE, text=True)
        output = result.stdout.strip().split('\n')
        gpu_info = []
        for line in output:
            gpu_index, power_draw, memory_used, gpu_util, memory_free = line.split(',')
            gpu_info.append({'GPU Index': int(gpu_index), 'Power Draw (W)': float(power_draw), 'Memory Used (MB)': float(memory_used), 'GPU Utilization (%)': float(gpu_util), 'Memory Free (MB)': float(memory_free)})
        return gpu_info
...
def set_available_gpus(gpu_info, num_gpus):
    ...
    if len(available_gpus) < num_gpus:
        raise ValueError(f"Not enough available GPUs! Available GPUs: {available_gpus}, Required number of GPUs: {num_gpus}")
    else:
