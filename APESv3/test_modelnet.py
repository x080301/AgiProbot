...
def test(local_rank, config):
    ...
    with torch.no_grad():
        ...
        for i, (samples, cls_labels) in enumerate(test_loader):
            ...
            print(f'idx 0:{len(vis_test_gather_dict["trained"]["idx"][0][0])}')
            print(f'idx 1:{len(vis_test_gather_dict["trained"]["idx"][1][0])}')
            print(f'attention_point_score 0:{len(vis_test_gather_dict["trained"]["attention_point_score"][0][0])}')
            print(f'attention_point_score 1:{len(vis_test_gather_dict["trained"]["attention_point_score"][1][0])}')
            exit()
