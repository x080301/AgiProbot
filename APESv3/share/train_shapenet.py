...
def train(local_rank, config):  # the first arg must be local rank for the sake of using mp.spawn(...)
    ...
    if config.datasets.dataset_name == 'shapenet_Yi650M':
        ...
        train_set, validation_set, trainval_set, test_set = dataloader.get_shapenet_dataset_Normal(config.datasets, config.train.dataloader, config.train.dataloader.vote)
    else:
        raise ValueError('Not implemented!')
