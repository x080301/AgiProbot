...
class EdgeConv(nn.Module):
    ...
    def forward(self, x):
        # x.shape == (B, C, N)
        x, _ = ops.group(x, self.K, self.group_type)
...
class DownSample(nn.Module):
    def __init__(self, M, q_in, q_out, k_in, k_out, v_in, v_out, num_heads):
    ...
    def forward(self, x):
        ...
        attention = self.softmax(energy / scale_factor)
        # attention.shape == (B, H, N, N)
        self.idx = torch.sum(attention, dim=-2).topk(self.M, dim=-1)[1]
        # idx.shape == (B, H, K)
...
class DownSampleWithSigma(nn.Module):
    def __init__(self, M, q_in, q_out, k_in, k_out, v_in, v_out, num_heads):
        ...
        self.M = M  # number of downsampled points
    ...
    def forward(self, x):
        ...
        attention = self.softmax(energy / scale_factor)
        # attention.shape == (B, H, N, 1, K)
        self.idx = torch.std(attention, dim=-1, unbiased=False)[:, :, :, 0].topk(self.M, dim=-1)[1]
        # idx.shape == (B, H, M)
        idx_dropped = torch.std(attention, dim=-1, unbiased=False)[:, :, :, 0].topk(attention.shape[-3]-self.M, dim=-1, largest=False)[1]
...
class ComparisonModel(nn.Module):
    def __init__(self, neighbor2point_enable, neighbor2point_egdeconv_emb_K, neighbor2point_egdeconv_emb_group_type, neighbor2point_egdeconv_emb_conv1_in,
                 ...
                 neighbor2point_down_which, neighbor2point_downsample_M, neighbor2point_down_q_in, neighbor2point_down_q_out, neighbor2point_down_k_in, neighbor2point_down_k_out, neighbor2point_down_v_in, neighbor2point_down_v_out,
                 ...
                 egdeconv_emb_conv1_out, egdeconv_emb_conv2_in, egdeconv_emb_conv2_out, edgeconv_downsample_which, edgeconv_downsample_M, edgeconv_downsample_q_in, edgeconv_downsample_q_out,
