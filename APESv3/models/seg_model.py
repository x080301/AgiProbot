...
class ShapeNetModel_inference_time(nn.Module):
    def __init__(self, config):

        super(ShapeNetModel_inference_time, self).__init__()

        # num_enabled_blocks = neighbor2point_enable + point2point_enable + edgeconv_enable
        # if num_enabled_blocks != 1:
        #     raise ValueError(f'Only one of feature_learning_block, point2point_block and edgecov_block should be enabled, but got {num_enabled_blocks} block(s) enabled!')
        if config.feature_learning_block.enable:
            self.block = seg_block.FeatureLearningBlock_inference_time(
...
class ShapeNetModel_fps(nn.Module):
    def __init__(self, config):

        super(ShapeNetModel_fps, self).__init__()

        # num_enabled_blocks = neighbor2point_enable + point2point_enable + edgeconv_enable
        # if num_enabled_blocks != 1:
        #     raise ValueError(f'Only one of feature_learning_block, point2point_block and edgecov_block should be enabled, but got {num_enabled_blocks} block(s) enabled!')
        if config.feature_learning_block.enable:
            self.block = seg_block.FeatureLearningBlock_fps(
                config.feature_learning_block
            )
            output_channels = (
                config.feature_learning_block.attention.ff_conv2_channels_out[-1]
...
class ShapeNetModel_inference_time_fps(nn.Module):
    def __init__(self, config):

        super(ShapeNetModel_inference_time_fps, self).__init__()

        # num_enabled_blocks = neighbor2point_enable + point2point_enable + edgeconv_enable
        # if num_enabled_blocks != 1:
        #     raise ValueError(f'Only one of feature_learning_block, point2point_block and edgecov_block should be enabled, but got {num_enabled_blocks} block(s) enabled!')
        if config.feature_learning_block.enable:
            self.block = seg_block.FeatureLearningBlock_inference_time_fps(
                config.feature_learning_block
            )
            output_channels = (
                config.feature_learning_block.attention.ff_conv2_channels_out[-1]
...
class ShapeNetModel_fpsknn(nn.Module):
    def __init__(self, config):

        super(ShapeNetModel_fpsknn, self).__init__()

        # num_enabled_blocks = neighbor2point_enable + point2point_enable + edgeconv_enable
        # if num_enabled_blocks != 1:
        #     raise ValueError(f'Only one of feature_learning_block, point2point_block and edgecov_block should be enabled, but got {num_enabled_blocks} block(s) enabled!')
        if config.feature_learning_block.enable:
            self.block = seg_block.FeatureLearningBlock_fpsknn(
                config.feature_learning_block
            )
            output_channels = (
                config.feature_learning_block.attention.ff_conv2_channels_out[-1]
...
class ShapeNetModel_inference_time_fpsknn(nn.Module):
    def __init__(self, config):

        super(ShapeNetModel_inference_time_fpsknn, self).__init__()

        # num_enabled_blocks = neighbor2point_enable + point2point_enable + edgeconv_enable
        # if num_enabled_blocks != 1:
        #     raise ValueError(f'Only one of feature_learning_block, point2point_block and edgecov_block should be enabled, but got {num_enabled_blocks} block(s) enabled!')
        if config.feature_learning_block.enable:
            self.block = seg_block.FeatureLearningBlock_inference_time_fpsknn(
                config.feature_learning_block
            )
            output_channels = (
                config.feature_learning_block.attention.ff_conv2_channels_out[-1]
