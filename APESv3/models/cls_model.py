...
class ModelNetModel_inference_time(nn.Module):
    def __init__(self, config):

        super(ModelNetModel_inference_time, self).__init__()

        if config.feature_learning_block.enable:
            self.block = cls_block.FeatureLearningBlock_inference_time(
                config.feature_learning_block
            )
            num_layers = len(config.feature_learning_block.attention.K)
        else:
            raise ValueError("This time only support neighbor2point block!")
    ...
    def forward(self, x):  # x.shape == (B, 3, N)
        if self.res_link_enable:  # True
            ...
            if self.aux_loss_enable:  # False
                ...
                # no_aux
                x = self.MLP(x)  # x.shape == (B, 40)
                return x, inference_time_list
        else:
            # no_res_link
            x, inference_time_list = self.block(x)  # x.shape == (B, 1024)
            x = self.MLP_no_res(x)  # x.shape == (B, 40)
            return x, inference_time_list
...
class ModelNetModel_fps(nn.Module):
    def __init__(self, config):

        super(ModelNetModel_fps, self).__init__()

        if config.feature_learning_block.enable:
            self.block = cls_block.FeatureLearningBlock_fps(config.feature_learning_block)
            num_layers = len(config.feature_learning_block.attention.K)
        else:
            raise ValueError("This time only support neighbor2point block!")
...
class ModelNetModel_fps_inference_time(nn.Module):
    def __init__(self, config):

        super(ModelNetModel_fps_inference_time, self).__init__()

        if config.feature_learning_block.enable:
            self.block = cls_block.FeatureLearningBlock_fps_inference_time(
                config.feature_learning_block
            )
            num_layers = len(config.feature_learning_block.attention.K)
        else:
...
class ModelNetModel_fpsknn(nn.Module):
    def __init__(self, config):

        super(ModelNetModel_fpsknn, self).__init__()

        if config.feature_learning_block.enable:
            self.block = cls_block.FeatureLearningBlock_fpsknn(
                config.feature_learning_block
            )
            num_layers = len(config.feature_learning_block.attention.K)
        else:
...
class ModelNetModel_fpsknn_inference_time(nn.Module):
    def __init__(self, config):

        super(ModelNetModel_fpsknn_inference_time, self).__init__()

        if config.feature_learning_block.enable:
            self.block = cls_block.FeatureLearningBlock_fpsknn_inference_time(
                config.feature_learning_block
            )
            num_layers = len(config.feature_learning_block.attention.K)
        else:
