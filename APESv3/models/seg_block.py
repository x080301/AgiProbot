...
class Neighbor2PointAttentionBlock(nn.Module):
    def __init__(self, config_n2p_block):
        ...
        if upsample_which == 'crossA':
            self.upsample_list = nn.ModuleList([upsample.UpSample(config_n2p_block.upsample, layer) for layer in
                                                range(len(config_n2p_block.upsample.q_in))])
        elif upsample_which == 'selfA':
            self.upsample_list = nn.ModuleList(
                [upsample.UpSampleSelfAttention(config_n2p_block.upsample, layer) for layer in
                 range(len(config_n2p_block.upsample.q_in))])
        elif upsample_which == 'interpolation':
            self.upsample_list = nn.ModuleList(
                [upsample.UpSampleInterpolation(config_n2p_block.upsample, layer) for layer in
                 range(len(config_n2p_block.upsample.q_in))])
        else:
