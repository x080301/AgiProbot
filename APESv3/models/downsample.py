...
from utils.ops import calculate_num_points_to_choose, bin_partition, generating_downsampled_index
...
def bin_probability_multiple(x_ds, input_x_shape, down_sampling_idx, bin_chunks_idx, bin_probability, direct_link_mode):
    ...
    if direct_link_mode == 'no_link' or direct_link_mode == 'no_link_no_sigmoid' or direct_link_mode == 'raw_gradient':
        bin_probability = bin_probability / M + 1.0
    elif direct_link_mode == 'no_link_higher_gradient' or direct_link_mode == 'higher_gradient':
    ...
    for i in range(num_bins):
        for j in range(B):
            ...
            tensor_to_multiply[j, :][bin_chunks_idx[i][j].flatten()] = bin_probability[j, i]
...
class DownSampleCarve(nn.Module):
    ...
    def idx_selection(self, x):
        ...
        if self.idx_mode == "col_sum":
            attention_point_score = torch.sum(self.attention_map,
...
class DownSampleToken(nn.Module):
    ...
    def output_variable_calculatio(self):
        ...
        self.idx_chunks = [[index_point[(index_bin == i) & (index_batch == j)].reshape(1, -1)
                            for j in range(B)]
                           for i in range(num_bins)]
