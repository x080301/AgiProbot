...
class ModelNetModel_all_inference_time(nn.Module):
    def __init__(self, config):

        super(ModelNetModel_all_inference_time, self).__init__()

        if config.feature_learning_block.enable:
            self.block = FeatureLearningBlock_all_inference_time(config.feature_learning_block)
            num_layers = len(config.feature_learning_block.attention.K)
        else:
            raise ValueError('This time only support neighbor2point block!')
    ...
    def forward(self, x):  # x.shape == (B, 3, N)
        ...
        if self.res_link_enable:
            # with res_link
            x, x_res_link_list, (time_before_ds_0, time_ds_0, time_ds_1) = self.block(x)  # x.shape == (B, 3C)
            if self.aux_loss_enable:
                raise NotImplementedError
            else:
                # no_aux
                ...
                model_inference_time_total = time.time() - time_begin
                model_inference_time_start_to_1stds = time_ds_0 - time_begin
                model_inference_time_start_to_2ndds = time_ds_1 - time_begin
                model_inference_time_dsonly = time_ds_0 - time_before_ds_0
                return x, (
                    model_inference_time_total,
                    model_inference_time_start_to_1stds,
                    model_inference_time_start_to_2ndds,
                    model_inference_time_dsonly)
...
class FeatureLearningBlock_all_inference_time(nn.Module):
    def __init__(self, config_feature_learning_block):
        ...
        super(FeatureLearningBlock_all_inference_time, self).__init__()
        self.embedding_list = nn.ModuleList(
            [embedding.EdgeConv(config_feature_learning_block.embedding, layer) for layer in
             range(len(config_feature_learning_block.embedding.K))])
        if downsample_which == 'global':
    ...
    def forward(self, x):
        ...
        if self.res_link_enable:
            ...
            self.res_link_list = res_link_list
            x = torch.cat(res_link_list, dim=1)
            return x, res_link_list, (time_before_ds_0, time_ds_0, time_ds_1)
        else:
            for i in range(len(self.downsample_list)):
