...
class FeatureLearningBlock_to_2nd_ds(nn.Module):
    def __init__(self, config_feature_learning_block):
        super(FeatureLearningBlock_to_2nd_ds, self).__init__()
        downsample_which = config_feature_learning_block.samble_downsample.ds_which
        upsample_which = config_feature_learning_block.upsample.us_which
    ...
    def forward(self, x):
        ...
        for i in range(len(self.downsample_list)):
            ...
            if i == len(self.downsample_list) - 1:
                x = self.feature_learning_layer_list[i + 1](x)
                x_xyz = ops.gather_by_idx(x_xyz, idx_select)
...
class ShapeNetModel_to_2nd_ds(nn.Module):
    def __init__(self, config):

        super(ShapeNetModel_to_2nd_ds, self).__init__()

        # num_enabled_blocks = neighbor2point_enable + point2point_enable + edgeconv_enable
        # if num_enabled_blocks != 1:
        #     raise ValueError(f'Only one of feature_learning_block, point2point_block and edgecov_block should be enabled, but got {num_enabled_blocks} block(s) enabled!')
        if config.feature_learning_block.enable:
            self.block = FeatureLearningBlock_to_2nd_ds(config.feature_learning_block)
            output_channels = config.feature_learning_block.attention.ff_conv2_channels_out[-1]
        # if point2point_enable:
        #     self.block = seg_block.Point2PointAttentionBlock(point2point_egdeconv_emb_K, point2point_egdeconv_emb_group_type,
        #                                            point2point_egdeconv_emb_conv1_in, point2point_egdeconv_emb_conv1_out, point2point_egdeconv_emb_conv2_in, point2point_egdeconv_emb_conv2_out,
...
class FeatureLearningBlock_to_1st_ds(nn.Module):
    def __init__(self, config_feature_learning_block):
        super(FeatureLearningBlock_to_1st_ds, self).__init__()
        downsample_which = config_feature_learning_block.samble_downsample.ds_which
        upsample_which = config_feature_learning_block.upsample.us_which
...
class ShapeNetModel_to_1st_ds(nn.Module):
    def __init__(self, config):

        super(ShapeNetModel_to_1st_ds, self).__init__()

        # num_enabled_blocks = neighbor2point_enable + point2point_enable + edgeconv_enable
        # if num_enabled_blocks != 1:
        #     raise ValueError(f'Only one of feature_learning_block, point2point_block and edgecov_block should be enabled, but got {num_enabled_blocks} block(s) enabled!')
        if config.feature_learning_block.enable:
            self.block = FeatureLearningBlock_to_1st_ds(config.feature_learning_block)
            output_channels = config.feature_learning_block.attention.ff_conv2_channels_out[-1]
        # if point2point_enable:
        #     self.block = seg_block.Point2PointAttentionBlock(point2point_egdeconv_emb_K, point2point_egdeconv_emb_group_type,
        #                                            point2point_egdeconv_emb_conv1_in, point2point_egdeconv_emb_conv1_out, point2point_egdeconv_emb_conv2_in, point2point_egdeconv_emb_conv2_out,
...
class FeatureLearningBlock_ds_only(nn.Module):
    def __init__(self, config_feature_learning_block):
        super(FeatureLearningBlock_ds_only, self).__init__()
        downsample_which = config_feature_learning_block.samble_downsample.ds_which
        upsample_which = config_feature_learning_block.upsample.us_which
...
class ShapeNetModel_ds_only(nn.Module):
    def __init__(self, config):

        super(ShapeNetModel_ds_only, self).__init__()

        # num_enabled_blocks = neighbor2point_enable + point2point_enable + edgeconv_enable
        # if num_enabled_blocks != 1:
        #     raise ValueError(f'Only one of feature_learning_block, point2point_block and edgecov_block should be enabled, but got {num_enabled_blocks} block(s) enabled!')
        if config.feature_learning_block.enable:
            self.block = FeatureLearningBlock_ds_only(config.feature_learning_block)
            output_channels = config.feature_learning_block.attention.ff_conv2_channels_out[-1]
        # if point2point_enable:
        #     self.block = seg_block.Point2PointAttentionBlock(point2point_egdeconv_emb_K, point2point_egdeconv_emb_group_type,
        #                                            point2point_egdeconv_emb_conv1_in, point2point_egdeconv_emb_conv1_out, point2point_egdeconv_emb_conv2_in, point2point_egdeconv_emb_conv2_out,
    ...
    def forward(self, x, category_id):
        ...
        x_tmp, cost_time = self.block(x)

        return x_tmp, cost_time
