...
def train(args, io):
    ...
    if args.use_sgd:
        print("Use SGD")
        opt = optim.SGD([{'params': params1, 'initial_lr': args.lr}], lr=args.lr, momentum=args.momentum, weight_decay=1e-2)
        opt2 = optim.SGD([{'params': params2, 'initial_lr': args.lr}], lr=args.lr, momentum=args.momentum, weight_decay=1e-2)
    else:
        print("Use Adam")
    ...
    if args.scheduler == 'cos':
        ...
        scheduler2 = StepLR(opt2, 20, 0.1, args.epochs)
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        for i,(points,target,class_label,goals,_,type_label) in tqdm(enumerate(train_loader),total=len(train_loader),smoothing=0.9):
            points, target,class_label,goals,type_label = points.to(device), target.to(device) ,class_label.to(device),goals.to(device),type_label.to(device)       #(batch_size, num_points, features)    (batch_size, num_points)
            ...
            class_pred,type_pred=tail1(embedding)
            ## classes typeA typeB 
            loss_class = criterion(class_pred, class_label,weights__,using_weight=args.use_class_weight)
            ## with cover  and no
            loss_type = criterion(type_pred, type_label,weights__,using_weight=args.use_class_weight)
            loss1 =feature_transform_reguliarzer(trans)*args.stn_loss_weight+loss_class+loss_type
            loss1.backward()
            opt.step()
            ## classes typeA typeB 
            batch_class_label = class_label.view(-1, 1)[:, 0].cpu().data.numpy() 
            pred_class_choice = class_pred.cpu().data.max(1)[1].numpy()  #array(batch_size*num_points)
            correct_class = np.sum(pred_class_choice == batch_class_label)
            ...
            pred_type_choice = type_pred.cpu().data.max(1)[1].numpy()  #array(batch_size*num_points)
            correct_type = np.sum(pred_type_choice == batch_type_label)
            ...
            opt2.zero_grad()
            embedding,before_embedding,trans = head(points.float())        #(batch_size, class_categories, num_points)
            type_one_hot = F.one_hot(class_label.reshape(-1).long(), num_classes=5)
            num_one_hot = F.one_hot(type_label.reshape(-1).long(), num_classes=2)
            ...
            loss2 = loss2+feature_transform_reguliarzer(trans)*args.stn_loss_weight
            loss2.backward()
            opt2.step()
        ...
        with torch.no_grad():         
            ...
            for i,(points,seg,class_label_,type_label_) in tqdm(enumerate(test_loader),total=len(test_loader),smoothing=0.9):
                points, seg,class_label_,type_label_ = points.to(device), seg.to(device),class_label_.to(device),type_label_.to(device)
                points=normalize_data(points)
                ...
                class_pred_,type_pred_=tail1(embedding_)
                ## classes typeA typeB 
                loss_class_ = criterion(class_pred_, class_label_,weights__,using_weight=args.use_class_weight)
                ## with cover  and no
                loss_type_ = criterion(type_pred_, type_label_,weights__,using_weight=args.use_class_weight)
                loss1 =loss_class_+loss_type_

                ## classes typeA typeB 
                batch_class_label_= class_label_.view(-1, 1)[:, 0].cpu().data.numpy() 
                pred_class_choice_ = class_pred_.cpu().data.max(1)[1].numpy()  #array(batch_size*num_points)
                correct_class_ = np.sum(pred_class_choice_ == batch_class_label_)
                total_correct_classification += correct_class_
                total_seen_classification += (batch_size)
                ## with cover  and no
                batch_type_label_ = type_label_.view(-1, 1)[:, 0].cpu().data.numpy() 
                pred_type_choice_ = type_pred_.cpu().data.max(1)[1].numpy()  #array(batch_size*num_points)
                correct_type_ = np.sum(pred_type_choice_ == batch_type_label_)
                total_correct_type += correct_type_
                total_seen_type += (batch_size)


                type_one_hot_ = F.one_hot(class_label_.reshape(-1), num_classes=5)
                num_one_hot_ = F.one_hot(type_label_.reshape(-1), num_classes=2)
               ...
               ####### calculate claciffication ##############
                for l in range(NUM_CLASS_MOTOR):
                   cla_seen_class[l] += np.sum((batch_class_label_ == l))
                   cla_correct_class[l] += np.sum((pred_class_choice_ == l) & (batch_class_label_ == l))
                   cla_iou_deno_class[l] += np.sum(((pred_class_choice_ == l) | (batch_class_label_ == l)))

               ####### calculate claciffication2 ##############
                for l in range(NUM_CLASS_MOTOR):
                   cla2_seen_class[l] += np.sum((batch_type_label_ == l))
                   cla2_correct_class[l] += np.sum((pred_type_choice_ == l) & (batch_type_label_ == l))
                   cla2_iou_deno_class[l] += np.sum(((pred_type_choice_ == l) | (batch_type_label_ == l)))
            ...
            io.cprint('\n\n')
        writer.add_scalar('learning rate',opt2.param_groups[0]['lr'],  epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size', type=int, default=3, metavar='batch_size',
                        help='Size of batch)')
    parser.add_argument('--data_dir', type=str, default='/home/bi/study/thesis/data/synthetic/merge', 
                        help='file need to be tested')
    parser.add_argument('--which_dataset', type=str, default='Dataset3_10type', metavar='N',
