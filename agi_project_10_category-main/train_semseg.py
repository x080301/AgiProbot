...
def train(args, io):
    ...
    VALIDATION_SET = MotorDataset_validation(split='test', data_root=args.data_dir,num_class=NUM_CLASS, num_points=NUM_POINT, test_area=args.validation_symbol, sample_rate=1.0, transform=None)
    ...
    validation_loader = DataLoader(VALIDATION_SET, num_workers=8, batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        for i,(points,target,type_label,goals,masks,type) in tqdm(enumerate(train_loader),total=len(train_loader),smoothing=0.9):
            points, target,type_label,goals,masks = points.to(device), target.to(device) ,type_label.to(device),goals.to(device) ,masks.to(device)       #(batch_size, num_points, features)    (batch_size, num_points)
            ...
            opt.zero_grad()
            seg_pred,trans,class_pred,result = model(points.float(),target)        #(batch_size, class_categories, num_points)
            ...
            loss = criterion(seg_pred.view(-1, NUM_CLASS), target.view(-1,1).squeeze(),weights,using_weight=args.use_class_weight)     #a scalar
        ...
        outstr = 'Classification:Train %d, loss: %.6f, train acc: %.6f ' % (epoch,(loss_class / num_batches),(total_correct_classification / float(total_seen_classification)))
        io.cprint(outstr)
        ...
        with torch.no_grad():         
            ...
            for i,(points,seg,type_label_,_) in tqdm(enumerate(validation_loader),total=len(validation_loader),smoothing=0.9):
                ...
                loss = criterion(seg_pred_.view(-1, NUM_CLASS), seg.view(-1,1).squeeze(),weights,using_weight=args.use_class_weight)     #a scalar
...
def test(args, io):
    ...
    try:
        if args.finetune:
            if os.path.exists(str(BASE_DIR)+"/outputs/"+args.model+'/'+args.which_dataset+'/'+args.exp_name+add_string+"/models/best_finetune.pth"):
                model_path=str(BASE_DIR)+"/outputs/"+args.model+'/'+args.which_dataset+'/'+args.exp_name+add_string+"/models/best_finetune.pth"
                checkpoint = torch.load(model_path)
                model.load_state_dict(checkpoint['model_state_dict'])
        else:
            inter=str(BASE_DIR)+"/outputs/"+args.model+'/'+args.which_dataset+'/'+args.exp_name+add_string+"/models/best_finetune.pth"
